...
//die();
$RequestedMethord = trim(DecData($_POST["mode"], 1, $objBF));
...
// Get List of Properties
	...
	$Property_id 		= trim($_POST["prop_reg_id"]);
	$Floor_id 			= trim($_POST["f_id"]);
	...
	if(!$vResult){
		...
		while($ListOfProperties = $objQayadProerty->dbFetchArray(1)){
					...
					$objQayadapplication->setProperty("ORDERBY", 'property_id');
					...
					if($objQayadapplication->totalRecords() > 0){
						...
						$objQayadapplication->setProperty("aplic_id", $ApplicationDetail["aplic_id"]);
						$objQayadapplication->lstPaymentOverview();
						$AplicPaymentOverView = $objQayadapplication->dbFetchArray(1);
						
						$objQayaduser->resetProperty();
						$objQayaduser->setProperty("user_id", $ListOfProperties["seller_agent_id"]);
...
// Get List of Lock Properties Agent Base
	...
	$acc_token		= trim($_POST["acc_token"]);
	...
	// Post Property Lock 
	$acc_token				= trim($_POST["acc_token"]);
	$property_id			= trim($_POST["prop_id"]);
	...
	$till_lock_duration		= trim($_POST["till_lock_duration"]);
	...
	$GetCustomerCnic = trim($_POST["customer_cnic"]);
	$objQayadapplication->resetProperty();
